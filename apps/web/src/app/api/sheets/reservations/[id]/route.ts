...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { sheetId, changed_by, ...data } =
      await req.json() as { sheetId?: string; changed_by?: string } & Partial<Omit<Reservation, 'reservation_id' | 'created_at'>>;
    if (!sheetId || !changed_by)
      return NextResponse.json({ error: 'sheetId, changed_by 필요' }, { status: 400 });
    const updated = await svc.updateReservation(sheetId, params.id, data, changed_by);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await svc.cancelReservation(sheetId, params.id, cancelled_by);
